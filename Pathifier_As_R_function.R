...
# Modificado de: Miguel Angel Garcia Campos github: https://github.com/AngelCampos
################################################################################
...
# Filter_value=3.75
#Matrix_Top_N <- args[8]
# Matrix_Top_N = 50 ; M.matrix = as.matrix(list_of_submatrices_withcontrols[[k]]) ; gene_sets_path = paste0(path_to_KEGG,"KEGG_pathways_in_df_genesymbol.tsv") ; Path_of_Code = Path_of_Code ;Path_of_Results = Path_of_Results ;Tumour_subtype = paste0("Pathifier_",Labels) ;Stabilizing =5  ; Filter_value = 3 ;Matrix_Top_N =50 

Pathifier_As_R_Function <- function(M.matrix, gene_sets_path, Path_of_Code, Path_of_Results, Tumour_subtype,Stabilizing,Filter_value,Matrix_Top_N ){
#######################################################################
...
Matrix_Top_N <- as.numeric(Matrix_Top_N)
###############################################################################
...
Path_of_Results <- paste0(Path_of_Results,"/")
...
write(min_std, file = paste(Path_of_Results,Tumour_subtype,c("M_min_std.txt"),sep = ""))
write.table(matrix, file = paste(Path_of_Results,Tumour_subtype,c("M_BRCA_Pathifier.txt"),sep = ""), quote = F, sep = "\t", 
            col.names = NA)
write.table(x = NORMALS, file = paste(Path_of_Results,Tumour_subtype,c("M_NORMALS.txt"),sep=""), quote = F, sep = "\t", col.names = F)
write(genes.m, file = paste(Path_of_Results,Tumour_subtype,c("genes_M.txt"),sep=""), sep = "\t")
...
# Plotting the top N
plot_raw_Matrix_png_NO_clustering(median_dfz_ordered[1:Matrix_Top_N,],Tumour_subtype,paste(Path_of_Results,Tumour_subtype,"_median_PDSz_ordered_matrix_Top",Matrix_Top_N,".png",sep=""))

matrix_txt_path<-paste(Path_of_Results,Tumour_subtype,"_median_PDSz_ordered_matrix_Top",Matrix_Top_N,".txt",sep="")
write.table(median_dfz_ordered[1:Matrix_Top_N,],file=matrix_txt_path,quote=FALSE,sep="\t",row.names = TRUE,col.names = TRUE)
list_for_result <- list(median_dfz_ordered[1:Matrix_Top_N,],PDSmatrix)
names(list_for_result) <- c("Top50","Full-PDS-matrix")
